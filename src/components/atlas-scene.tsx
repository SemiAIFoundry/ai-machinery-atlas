...
export default function AtlasScene(props:Props){
 ...
 useEffect(()=>{
  ...
  const mx=new T.Matrix4();function tick(now:number){frame=requestAnimationFrame(tick);const dt=Math.max(0,Math.min((now-last)/1000,.05));last=now;const p=live.current;if(p.running&&!motion&&!document.hidden)time+=dt;

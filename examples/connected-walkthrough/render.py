#!/usr/bin/env python3
"""Reproduce the captured 60-second edit from its preserved UI captures.

Requires Python 3.10+, Pillow and FFmpeg with libx264/AAC. The captured
...
def verify_inputs() -> dict:
    ...
    if len(frames) != 10 or manifest["durationS"] != 60:
        raise ValueError("Expected the preserved 10-capture, 60-second edit")
    if manifest["resolution"] != [1920, 1080] or manifest["fps"] != 30:
        raise ValueError("Unexpected captured output dimensions or frame rate")
    start = 0
    names = set()
    for row in frames:
        name = row["file"]
    ...
    with wave.open(str(HERE.parent / "walkthrough" / "original-score.wav"), "rb") as score:
...
def render_frames(manifest, output, font_path):
    ...
    (output / "frames.txt").write_text(concat + "file 'rendered/009.png'\n")
...
def main():
    ...
    if args.verify_only:
        print("Verified 10 capture hashes, contiguous 60-second edit, original score and font.")
        return
    output = args.output.resolve()
    repo_public = HERE.parent.parent / "public"
    if output == repo_public or repo_public in output.parents:
        parser.error("Reproduction output must be outside public; the captured media is immutable")
    if output.exists():
        parser.error("Output already exists; choose a new directory to preserve prior outputs")
    font_path = Path(os.environ.get("FONT_FILE", str(HERE.parent / "walkthrough" / "fonts/geist-latin.woff2"))).resolve()
    ...
    report = {
        ...
        "capturedArtifactSha256": manifest["artifactSha256"],
        "font": {"filename": font_path.name, "sha256": sha256(font_path)},
        "note": "Captured titles use the bundled Geist font. Reproduction uses the selected font. "
    ...
    if not args.render_only:
        ...
        report["artifactSha256"] = sha256(output / "connected.mp4")
    (output / "reproduction.json").write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps({"durationS": 60, "captures": 10,
